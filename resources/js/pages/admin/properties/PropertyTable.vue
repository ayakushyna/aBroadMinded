    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <BTable striped hover responsive
                    border no-border-collapse fixed
                    no-local-sorting
                    :items="table.items"
                ...
                <template v-slot:cell(actions)="row" >
                    <b-button-group>
                        <b-button  variant="outline-dark" :to="{name: 'ProfileEdit', params: {id: row.item.id}}">
        ...
        methods: {
            ...
            fetchData(page) {
                try {
                    ...
                    axios.get(this.$route.meta.api.properties + '/'+ this.$route.params.id + '/' + this.show + '/' + '?page=' + page, {
                        params: {
                        ...
                        .then(response => {
                            ...
                            let fields = response.data.fields.filter(field => field.key !== 'property');
                            for(let i in fields){
            ...
            async deleteItem(id) {
                if(this.show === 'profiles'){
                    this.axios
                        .delete(this.$route.meta.api.profiles + '/'+ id)
                        .then(response => {
                            let i = this.table.items.map(item => item.id).indexOf(id);
                            this.table.items.splice(i, 1)
                        });
                }
                else if(this.show === 'bookings'){
                    this.axios
                        .delete(this.$route.meta.api.bookings + '/'+ id)
                        .then(response => {
                            let i = this.table.items.map(item => item.id).indexOf(id);
                            this.table.items.splice(i, 1)
                        });
                }
                else if(this.show === 'feedbacks'){
                    this.axios
                        .delete(this.$route.meta.api.feedbacks + '/'+ id)
                        .then(response => {
                            let i = this.table.items.map(item => item.id).indexOf(id);
                            this.table.items.splice(i, 1)
                        });
                }
                else if(this.show === 'calendars'){
                    this.axios
                        .delete(this.$route.meta.api.calendars + '/'+ id)
                        .then(response => {
                            let i = this.table.items.map(item => item.id).indexOf(id);
                            this.table.items.splice(i, 1)
                        });
